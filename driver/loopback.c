...
// loopback_queue_entry stores the data body and accompanying metadata.
struct loopback_queue_entry {
    uint16_t type;
    size_t len;
    // flexible array member.
    // unknown size array that can be placed only at the end of the structure.
    // accessible as a member variable but not included in the structure size (be sure to allocate memory including the data portion).
    uint8_t data[];
};
...
static int loopback_transmit(struct net_device *dev, uint16_t type, const uint8_t *data, size_t len, const void *dst)
{
    ...
    // allocate memory for entries to be stored in the queue
    ...
    // set metadata and copy data itself.
    entry->type = type;
    entry->len = len;
